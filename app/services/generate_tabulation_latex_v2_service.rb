		 ...
		 a = ''
          ...
          Course.all.each do |course|
            if course.course_type === "lab"
                a << [data[course.code][:mo], data[course.code][:lg], data[course.code][:ps]].join(' & ') << ' & ' if data.include? course.code
             else
                a << [data[course.code][:cact], data[course.code][:fem], data[course.code][:mo], data[course.code][:lg], data[course.code][:ps]].join(' & ') << '&' if data.include? course.code
             end
          end

         a << [data[:tce], data[:tps], data[:gpa], data[:result], data[:remarks], data[:hall]].join(' & ')   #"\\multirow{3}{*}{#{data[:remarks]}}"
         a << "\\\\"
